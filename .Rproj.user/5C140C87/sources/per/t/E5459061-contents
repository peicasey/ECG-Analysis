...
getBaselines = function(windows) {
  ...
  for (window_pair in windows) {
    ...
    cycle_peaks = window[find_peaks(x=window$ecg, span = 20), ]
    print(cycle_peaks)
    cycles = list()
    for (c in 1:(length(cycle_peaks))) {
      cycle = data.frame()
      c_end = cycle_peaks

      for (p in 1:(nrow(window))) {
        # print(c_end)
        if (window[p,2] == c_end) { # checking the ecg matches the end point of a cycle
          break
        }

        cycle = rbind(cyle, windows[p,])
      }      


      # remove outliers and get mean
      z_scores = as.data.frame(sapply(cycle, function(cycle) (abs(cycle-mean(cycle))/sd(cycle))))
      no_outliers = z_scores[!rowSums(z_scores>3), ]
      mean = mean(no_outliers$ecg)

      time = cycle$time
      base = rep(mean, (length(cycle)))
